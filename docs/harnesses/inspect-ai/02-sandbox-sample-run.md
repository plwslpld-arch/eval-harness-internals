...
对普通问答，Sample 似乎只是 input 到 output；对 Agent Eval，Sample 还可能复制文件、执行 setup、创建 Sandbox、暴露工具、产生多轮消息、达到 token/time/cost limit，并在清理前检查终态。只看最终 output 会遗漏大量产品行为。本节追踪 `task_run` 如何把 Dataset Sample 变成 TaskState，再如何在隔离上下文中运行 Solver 和保存 EvalSample。

## 先建立源码地图

Sample 生命周期集中在锁定 [`_eval/task/run.py`](https://github.com/UKGovernmentBEIS/inspect_ai/blob/ebf4815ee260afcc8c34ad9d66e6f8d98a89e905/src/inspect_ai/_eval/task/run.py)。Task 级 SandboxManager 与并发准备在 [`_eval/run.py`](https://github.com/UKGovernmentBEIS/inspect_ai/blob/ebf4815ee260afcc8c34ad9d66e6f8d98a89e905/src/inspect_ai/_eval/run.py)。日志侧 `EvalSample`、SandboxEnvironmentSpec、EvalSampleLimit 与事件容器在 [`log/_log.py`](https://github.com/UKGovernmentBEIS/inspect_ai/blob/ebf4815ee260afcc8c34ad9d66e6f8d98a89e905/src/inspect_ai/log/_log.py)。
...
2. `create_sample_state` 把 Sample 输入、目标、Model、metadata 和初始消息装入 TaskState；每个新运行具有独立 UUID，避免 sample retry 复用错误状态。
3. `task_run_sample` 初始化 scoring context 和 Sandbox context。Sample 自己的 sandbox 可以覆盖 Task 默认；files、setup 与连接信息在环境初始化阶段处理。
4. 进入 Sandbox 后，Solver Plan 依次运行。Generate 函数更新 TaskState，模型调用、工具事件、store、消息和 output 进入 transcript/Event。
...
`TaskState` 是运行中的可变视图，包含 model、sample_id、epoch、input、messages、output、tools、store、metadata 与 scores。`EvalSample` 是日志中的持久化快照，除输入输出外还记录 target、sandbox、files、setup、events、scores、model_usage、error、limit 和时间摘要。

SandboxEnvironmentSpec 说明环境类型与配置，但真正的环境证据还包括连接、文件、命令结果和终态。EvalSampleLimit 要区分是 token、message、turn、time、working time 还是 cost 限制，因为这些停止原因对产品解释不同。
