...
OpenAI Evals 的经典实现围绕 Registry、EvalSpec、CompletionFn 和 Recorder 展开。命令行接收几个字符串，运行时却要把它们解析为 Eval 类、数据路径、模型调用对象和日志后端。新人若只看 YAML，会误以为配置本身就是执行；若只看某个 Eval 子类，又看不到运行身份和事件怎样统一记录。本课程追踪 `oaieval.run` 从配置键到 final report 的全过程。
...
2. CLI 合并 Eval 参数和 completion_args。CompletionFn 字符串按逗号拆分，由 `registry.make_completion_fn` 解析为模型快捷实现、Registry CompletionFn 或 Solver。
3. 入口构造 RunSpec，选择 Dummy、Local、HTTP 或数据库 Recorder，并通过 Registry 取得 Eval class。
4. Eval class 用 completion_fn_instances、EvalSpec args、eval_registry_path 和 Registry 实例化。数据相对路径会以该 Registry 的 data 目录为基准解析。
5. `eval.run(recorder)` 遍历样本。每个 Sample 进入 `recorder.as_default_recorder(sample_id)` 上下文，Eval/CompletionFn 通过 helper 写 sampling、match、function_call、metrics 或 error 事件。
6. Eval 返回聚合 dict；CLI 可从 sampling 事件补充 token usage，再调用 `record_final_report`。

## 关键数据结构

`EvalSpec` 保存 key、class、args 与 registry_path；`CompletionFnSpec` 保存类和参数；`RunSpec` 把 completion_fns、EvalSpec 和 run_id 固定为运行头。`Event` 至少把 run_id/sample_id/type/data/time 关联起来。CompletionFn Protocol 接收 prompt 并返回 CompletionResult，Eval 决定怎样把结果转成 match 或其他 metric。
...
HTTP Recorder 失败可回落 LocalRecorder，保护证据不因远端日志服务故障完全丢失；这属于记录层恢复，不代表产品 Sample 可以重答。CompletionFn 的网络 retry 与 Eval 的 Sample 语义也未在核心对象里形成显式 Trial/Attempt 分层。
