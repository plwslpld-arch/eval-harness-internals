...
RawRegistry 是 name 到原始 spec 的字典，而 BaseEvalSpec/EvalSpec/CompletionFnSpec 把 key、class 名、args、metrics 和 registry_path 结构化。逻辑 key 便于引用，class 决定代码，args 决定行为，registry_path 决定数据相对路径——四者缺一都不能构成完整身份。
...
多 Registry 路径支持私有 Eval 覆盖和扩展，但当前重复 key 选择失败而非后者覆盖。这是更安全的显式冲突策略。每次加载资源简单直观，却意味着性能和确定性依赖文件系统状态，因此要离线复现，就应该把解析结果和来源版本一起冻结。
...
若仅哈希逻辑 key，两个不同 Registry 的同名 Eval 会碰撞；而若只哈希 YAML 又忽略 CLI override，也无法代表实际运行。
...
确定解析对象不能证明 YAML 内容正确、数据授权充分或 class 无副作用，因为 Registry 解决配置发现与实例化，不替代 Dataset 治理、Sandbox 隔离或 Scorer 有效性。
