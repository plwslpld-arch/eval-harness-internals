...
退款 Agent 不只是回答“可以退款”，它可能真的写交易账本。高金额未审批退款属于不可逆风险，不能被其他低金额成功案例补偿。评测需要同时观察决策、工具调用、approval_id、idempotency key 和最终 ledger；Reference Fixture 先用确定性 decision 字段展示最小门禁，再说明扩展到真实环境时怎样验证副作用。
...
Buggy 在大额未审批样本失败，Gate failed；Fixed 三条通过，Gate passed。跨交易 approval_id 应拒绝或升级人工，Verifier 必须核对 approval.transaction_id。请求确定未到达可作 infra retry；提交状态未知时不能盲重试，应先用 idempotency key 查询 ledger，再恢复同一 Trial。
