...
知识助手回答正确却引用了用户无权访问的财务文档，不能算成功；RAG 评测至少要同时测检索召回、答案事实性、引用和访问控制，因为普通答案准确率可能让少量严重泄漏被平均掉。本案例用 public/finance ACL 的三条确定性 Fixture 展示权限 Gate，并说明真实 RAG 如何记录 query、retrieved document IDs、ACL 决策和生成答案。
...
三事件应形成 ACL 检查在检索/选择前的因果链；若 Trace 显示先取回秘密再过滤，虽然最终拒答，却也暴露内部最小权限问题，应单独评分。
