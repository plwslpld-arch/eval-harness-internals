...
开放式回答、合同风险和多轮 Agent 轨迹很难用精确匹配评分，LLM-as-a-Judge 因此常被直接加入评测。然而“让更强模型打 0 到 1 分”没有说明 rubric、输入可见性、输出 schema、失败/弃权、位置偏差、校准数据和成本。Judge 是有不确定性的测量器，不是事实裁判。本篇从 Reference Harness 的离线 Judge Protocol 出发，设计可替换但可核对的适配层。
...
Judge identity 与 Scorer identity 都必须版本化。Rubric 改一个词、模型别名漂移或输出解析器改变，都可能改变测量合同。`reason` 不应包含隐藏 chain-of-thought，只需给出可审计的判定依据、引用证据和 rubric 条目。若 Judge 可以 abstain，ScoreStatus 应表达 uncertain/unscorable，不把它压成 failed。
...
若 Judge 超时，应产生 Judge error/不可评分，不把产品计 0；若两位人工分歧且仲裁未完成，应是 uncertain；若 Judge 与人工在关键高风险条目上系统性漏判，即使整体一致率高也不能用于 release gate。
