...
预算至少区分 Harness infra attempts、Target 内部调用/工具预算、Judge 预算和全局墙钟预算。可恢复错误名单要白名单化；将所有 Exception 都视为可重试会掩盖确定性程序错误。取消不应自动继续；它可能来自用户、全局预算或上游任务撤销。
...
本地线程与单进程 canonical 规则不能证明分布式 worker 恰好一次、远端请求幂等或崩溃恢复无重复副作用。生产队列需额外 lease、fencing、幂等键与事务性提交。
