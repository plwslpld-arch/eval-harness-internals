...
shipping 有三个样本。buggy 通过 2/3，fixed 通过 3/3，点估计差为 1/3。因为两个 Target 使用完全相同的 Sample 和 repetition，可以计算每一对的 Score 差，再对配对差值重采样。样本只有三个，区间不会神奇地提供真实业务确定性；但配对至少避免把样本难度差异误当模型差异。发布 Gate 还要求 fixed pass-rate 达到 1.0 且没有 unscorable 证据。
