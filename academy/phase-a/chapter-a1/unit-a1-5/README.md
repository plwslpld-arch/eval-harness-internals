...
以许可英文合同、声明合同类型和司法辖区为总体；用跨条款、附件缺失、解析降级和困难负例补充风险。两位律师独立盲标并保留合理多值解释，第三位资深律师仲裁；交易包、修订链、附件、标准条款模板和时间截点共同切分。谈判草案不足使整体数据为 `partial`，不以现有样本数量掩盖。[完整案例](examples/contract-agent/evaluation-case.yaml)
